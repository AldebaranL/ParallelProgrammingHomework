...
{
public:
	explicit ANN_SIMD(int _SampleN, int nNIL, int nNOL, const int nNHL, float _sR = 0.2);
	~ANN_SIMD();

	void train_neon(int _sampleNum, float** _trainMat, float** _labelMat);
...
private:
	...
	void train_vec_neon(const float* _trainVec, const float* _labelVec, int index);
