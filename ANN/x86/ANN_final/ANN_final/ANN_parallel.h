#ifndef ANN_PARALLEL_H
#define ANN_PARALLEL_H
...
public:
    ANN_parallel(int* _num_each_layer, int _num_epoch = 10, int _batch_size = NUM_SAMPLE, int _num_layers = 1, float _study_rate = 0.1);
    ~ANN_parallel();
    ...
    void train_MPI(int _sampleNum, float** _trainMat, float** _labelMat);
    void train_openMP(int _sampleNum, float** _trainMat, float** _labelMat);
    void train_SIMD(int _sampleNum, float** _trainMat, float** _labelMat);
    void train_openMP_SIMD(int _sampleNum, float** _trainMat, float** _labelMat);
    void train_MPI_openMP_SIMD(int _sampleNum, float** _trainMat, float** _labelMat);
    void train_MPI_i(const int num_sample, float** _trainMat, float** _labelMat);
