...
ANN_3::ANN_3 ( int* _num_each_layer, int _num_epoch, int _batch_size, int _num_layers, float _study_rate)
{
...
ANN_3::~ANN_3()
{
    //printf ("begin free ANN_3");
    delete[]num_each_layer;
    for (int i = 0; i < layers.size(); i++) delete layers[i];
    // printf ("free ANN_3\n");
}
void ANN_3::shuffle (const int num_sample, float** _trainMat, float** _labelMat)
{
    //init
...
void ANN_3::train_SIMD  (const int num_sample, float** _trainMat, float** _labelMat)
{
...
void ANN_3::train (const int num_sample, float** _trainMat, float** _labelMat)
{
    printf ("begin training\n");
    float thre = 1e-2;
    float *avr_X = new float[num_each_layer[0]];
...
bool ANN_3::isNotConver_ (const int _sampleNum, float** _trainMat, float** _labelMat, float _thresh)
{
    float lossFunc = 0.0;
...
void ANN_3::get_predictions (float* X)
{
    predict (X);
    static int t = 0;
    printf ("in%d:", t);
