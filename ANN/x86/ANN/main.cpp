...
int main()
{
    ...
    ANN_3 ann ( (int*) NUM_EACH_LAYER, 128,1,NUM_LAYERS,0.1);
    ...
    ANN_openMP ann1 ( (int*) NUM_EACH_LAYER,  128,1,NUM_LAYERS,0.1);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &head); // start time
    ann1.train_SIMD(NUM_SAMPLE, TRAIN_MAT, LABEL_MAT);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &tail);	// end time
    cout << "SIMD:" << (tail - head)*1.0  / freq << "s" << endl;

    ANN_openMP ann2 ( (int*) NUM_EACH_LAYER,  128,1,NUM_LAYERS,0.1);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &head); // start time
    ann2.train_openMP(NUM_SAMPLE, TRAIN_MAT, LABEL_MAT);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &tail);	// end time
    cout << "openMP:" << (tail - head) *1.0 / freq << "s" << endl;

    ANN_openMP ann3 ( (int*) NUM_EACH_LAYER,  128,1,NUM_LAYERS,0.1);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &head); // start time
    ann3.train_SIMD_openMP(NUM_SAMPLE, TRAIN_MAT, LABEL_MAT);
    QueryPerformanceCounter ( (LARGE_INTEGER*) &tail);	// end time
    cout << "openMP+SIMD:" << (tail - head)*1.0  / freq << "s" << endl;
